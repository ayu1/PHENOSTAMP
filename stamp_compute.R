...
voronoiBinCount <- function(seeds, pred.mat) {
  p <- pdist(pred.mat, as.matrix(seeds))
  p.mat <- matrix(p@dist, nrow=p@p, ncol=p@n)
  p.groupmatch <- apply(p.mat, 2, which.min)
  return(table(p.groupmatch))
}
...
# Main PHENOSTAMP function
phenostamp <-function (tsnedat,asinhp,newdat,newdat2,antibody,nn,vor,
                    outputDir='./output', file1="Sample.tiff",sample="Sample") {
  Y1=tsnedat[,1]
  Y2=tsnedat[,2]
...
voronoi_mapping <- function(groups, ds, dbins, points_to_add=list()){
  ...
  for ( i in 1:length(targetclust)) {
    ...
    xysum <- bigvis::condense(bin(ds[id1,1], dbins[i] ), bin(ds[id1,2],dbins[i]))
...
optimalbinsize2 <- function(pop,timeid,ds,maxbinsize,targetclust=NULL) {
  ...
  for ( i in l3) {
    ...
    for ( k in c(1:bs) ){
      ...
      if(length(optstate) > 1) {
        id3=id1[grep(optstate,id2)]
        xysum <- bigvis::condense(bin(ds[id3,1], k ), bin(ds[id3,2],k))
